...
# This is to fix session error on Heroku
app.secret_key = "DEMO_SECRET_KEY2"
...
def main():
    if 'repoid' in session:
        ...
        tree = gt.getTree(session['repoid'])
        if tree is None:
...
def getDetails():
    ...
    if 'repoid' in session:
        data = json.loads(request.data.decode())
        response = gt.getFileDetails(session['repoid'],data['filepath'])
        return response, 200
    else:
        err = 'Please select a repository for analysis!'
        return render_template('index.html', errorstr = err)
